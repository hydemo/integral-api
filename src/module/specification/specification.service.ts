...
export class SpecificationService {
	constructor(
		@Inject('SpecificationModelToken')
		private readonly specificationModel: Model<ISpecification>,
	...
	async create(
		...
		const existing = await this.specificationModel.findOne({
		...
		await this.specificationModel.updateMany(
			{ sort: { $gte: specification.sort } },
			{ $inc: { sort: 1 } },
		);
	...
	async list(pagination: Pagination): Promise<IList<ISpecification>> {
		...
		const max = await this.specificationModel.countDocuments({
	...
	async findByIdAndRemove(id: string): Promise<boolean> {
		...
		await this.specificationModel.findByIdAndUpdate(id, {
			isDelete: true,
			deleteTime: Date.now(),
		});
		await this.specificationModel.updateMany(
			{ isDelete: false, sort: { $gte: existing.sort } },
			{ $inc: { sort: -1 } },
		);
