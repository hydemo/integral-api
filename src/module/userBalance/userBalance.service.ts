...
export class UserBalanceService {
  constructor(
    @Inject('UserBalanceModelToken') private readonly userBalanceModel: Model<IUserBalance>,
    @Inject(UserService) private readonly userService: UserService,
    @Inject(PaginationUtil) private readonly paginationUtil: PaginationUtil,
  ) { }

  // 创建数据
  async create(userBalance: CreateUserBalanceDTO): Promise<IUserBalance | null> {
  ...
  // 列表
  async listByUser(pagination: Pagination, user: string): Promise<IList<IUserBalance>> {
