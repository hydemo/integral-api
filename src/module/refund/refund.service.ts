...
import { CreateRefundDTO, UpdatePriceDTO, RefuseDTO, RefundDTO, RefundProductDTO } from './refund.dto';
...
export class RefundService {
  ...
  */
  ...
  // 生成退款
  async create(id: string, refund: CreateRefundDTO, user: string): Promise<Boolean> {
    ...
    const newProducts = refund.products.map(product => {
      const orderProduct = order.products.find(o => String(o.product) === product.product)
      if (product.count <= 0) {
      ...
      refundPrice += (orderProduct.realPrice - orderProduct.promoteMinus - orderProduct.discountMinus - orderProduct.vipMinus - orderProduct.preSaleMinus) * product.count
    ...
    const newRefund: RefundDTO = {
      ...
      redBagId: order.redBagId,
      redBagPrice: order.redBagPrice,
  ...
  async confirm(id: string, address: CreateAddressDTO) {
    const refund: IRefund | null = await this.refundModel.findById(id).lean().exec()
    if (!refund) {
    ...
    return await this.refundModel.findByIdAndUpdate(id, { ...address, checkResult: 2, confirmTime: Date.now() })
  }
  ...
  async cancel(id: string, user: string): Promise<IOrder | null> {
    ...
    if (refund.checkResult === 1 || refund.checkResult === 2) {
      ...
      refund.products.map(product => {
        const orderProduct = order.products.find(o => String(o.product) === String(product.product))
        if (!orderProduct) {
          throw new ApiException('产品不存在', ApiErrorCode.NO_EXIST, 404)
        }
        orderProduct.refundCount -= product.count
    ...
    if (refund.checkResult === 4 || refund.checkResult !== 5) {
      await this.refundModel.findByIdAndUpdate(id, { isDelete: true, deleteTime: Date.now() })
    }
    return null
  }

  // 根据id查找
  async changePrice(id: string, price: UpdatePriceDTO): Promise<IRefund | null> {
    return await this.refundModel.findByIdAndUpdate(id, price).lean().exec()
  }
  ...
  async refundOrder(id: string): Promise<IRefund | null> {
    ...
    let refundPrice = 0
    const newProducts: any = order.products.map(product => {
      refundPrice += (product.realPrice - product.promoteMinus) * (product.count - product.refundCount)
  ...
  async changeAddress(id: string, address: CreateAddressDTO): Promise<IOrder | null> {
    return await this.refundModel.findByIdAndUpdate(id, address).lean().exec()
  }
  ...
  async send(id: string, shipper: CreateShipperDTO, user: string) {
    ...
    await this.refundModel.findByIdAndUpdate(id, { sendTime: Date.now(), checkResult: 3 })
    return true
  }
  ...
  async refuse(id: string, refuse: RefuseDTO): Promise<IRefund | null> {
    ...
    await this.orderService.updateProduct(order._id, order.products)
    return await this.refundModel.findByIdAndUpdate(id, { refuseReason: refuse.refuseReason, checkResult: 5, refuseTime: Date.now() })
  }


  // 列表
  async listByUser(pagination: Pagination, user: string, checkResult?: number): Promise<IList<IRefund>> {
  ...
  async list(pagination: Pagination): Promise<IList<IRefund>> {
    const condition = this.paginationUtil.genCondition(pagination, ['refundSn', 'orderSn'], 'applicationTime')
    const list = await this.refundModel
      .find(condition)
      .limit(pagination.pageSize)
      .skip((pagination.current - 1) * pagination.pageSize)
  ...
  async handleRefund(order: IOrder, refund: IRefund) {
    ...
    if (order.payType === 2) {
      ...
      const result: any = await this.weixinUtil.refund(refund.refundPrice, order.actualPrice, order.user.weixinOpenid, order.paySn, refund._id)
      if (!result) {
    ...
    const newIntegration: CreateIntegrationDTO = {
      user: order.user,
      count: refund.refundPrice,
      type: 'minus',
      sourceType: 4,
      sourceId: order._id
    }
    await this.integrationService.refund(newIntegration)
    return await this.refundModel.findByIdAndUpdate(refund._id, { checkResult: 4, refundTime: Date.now(), refundId, refundType: order.payType })
  }
}