...
export class NoticeService {
  ...
  // 创建数据
  async create(createNoticeDTO: CreateNoticeDTO, user: string): Promise<INotice> {
    return await this.noticeModel.create({ ...createNoticeDTO, createBy: user })
  }
  ...
  // 修改数据
  async update(id: string, createNoticeDTO: CreateNoticeDTO): Promise<INotice | null> {
    return await this.noticeModel.findByIdAndUpdate(id, createNoticeDTO)
  }

  // 详情
  async detail(id: string): Promise<INotice> {
    const notice = await this.noticeModel.findById(id).lean().exec();
    if (!notice) {
  ...
  async all(): Promise<INotice[]> {
    return await this.noticeModel.find({ isDelete: false }).lean().exec();
  }

  // 软删除
  async remove(id: string, user: string) {
    return await this.noticeModel.findByIdAndUpdate(id, { isDelete: true, deleteTime: Date.now(), deleteBy: user });
  }

  // 恢复
  async recover(id: string) {
    return await this.noticeModel.findByIdAndUpdate(id, { isDelete: false, $unset: { deleteTime: 1, deleteBy: 1 } });
  }

  // 微信授权
  async oauth(oauth: OAuthDTO): Promise<any> {
    const notice: INotice | null = await this.noticeModel.findOne({ phone: oauth.phone })
    if (!notice) {
    ...
    if (openId) {
      // 更新微信信息
      await this.noticeModel.findByIdAndUpdate(notice._id, {
        openId,
        isWeixinNotice: true,
      }).lean().exec();
    }
