...
export class ServiceTemplateService {
	...
	create(path: string, name: string) {
		...
		codes.push(
			`    @Inject('${upCaseName}ModelToken') private readonly ${name}Model: Model<I${upCaseName}>,`,
		);
		codes.push(
		...
		codes.push(`  // 创建数据`);
		codes.push(
			`  async create(create${upCaseName}DTO: Create${upCaseName}DTO, user: string): Promise<I${upCaseName}> {`,
		);
		codes.push(
			`    return await this.${name}Model.create({ ...create${upCaseName}DTO, createBy: user })`,
		);
		codes.push(`  }\n`);

		codes.push(`  // 分页查询数据`);
		codes.push(
			`  async list(pagination: Pagination): Promise<IList<I${upCaseName}>> {`,
		);
		codes.push(
		...
		codes.push(`      .lean()`);
		codes.push(
			`    const total = await this.${name}Model.countDocuments(condition);`,
		);
		...
		codes.push(`  // 修改数据`);
		codes.push(
			`  async update(id: string, create${upCaseName}DTO: Create${upCaseName}DTO): Promise<I${upCaseName} | null> {`,
		);
		codes.push(
			`    return await this.${name}Model.findByIdAndUpdate(id, create${upCaseName}DTO)`,
		);
		codes.push(`  }\n`);
		...
		codes.push(
			`    return await this.${name}Model.findByIdAndUpdate(id, { isDelete: true, deleteTime: Date.now(), deleteBy: user });`,
		);
		codes.push(`  }\n`);
		...
		codes.push(
			`    return await this.${name}Model.findByIdAndUpdate(id, { isDelete: false, $unset: { deleteTime: 1, deleteBy: 1 } });`,
		);
		codes.push(`  }\n`);
		codes.push(`}`);
		fs.writeFileSync(
