...
export class SchemaTemplateService {
	...
	getType(type: string, schema: SchemaDTO) {
		...
		return `{ type: ${type}${enumString ? enumString : ''}${
	...
	create(path: string, name: string, schemas: SchemaDTO[]) {
		...
		schemas.map(schema => {
			...
			switch (schema.type) {
				...
				case 'phone':
					schemaCodes.push(
						`    ${schema.key}: ${this.getType('String', schema)},`,
					);
					break;
				case 'ObjectId':
					schemaCodes.push(
						`    ${schema.key}: ${this.getType('ObjectId', schema)},`,
					);
					break;
				case 'number':
					schemaCodes.push(
						`    ${schema.key}: ${this.getType('Number', schema)},`,
					);
					break;
				case 'date':
					schemaCodes.push(
						`    ${schema.key}: ${this.getType('Date', schema)},`,
					);
					break;
				case 'boolean':
					schemaCodes.push(
						`    ${schema.key}: ${this.getType('Boolean', schema)},`,
					);
					break;
				default:
					break;
