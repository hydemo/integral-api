...
export class DtoTemplateService {
  ...
  // 生成验证字段
  getSchema(schemaCodes: string[], schemaTypes: string[], validator: string, type: string, schema: SchemaDTO) {
    ...
    schemaCodes.push(`  @ApiProperty({ description: '${schema.description}' })`)
  ...
  create(path: string, name: string, schemas: SchemaDTO[]) {
    ...
    schemas.map(schema => {
      ...
      switch (schema.type) {
        case 'string':
          this.getSchema(schemaCodes, schemaTypes, 'IsString', 'string', schema)
          break;
        case 'ObjectId':
          this.getSchema(schemaCodes, schemaTypes, 'IsMongoId', 'string', schema)
          break;
        case 'number':
          this.getSchema(schemaCodes, schemaTypes, 'IsNumber', 'number', schema)
          break;
        case 'phone':
          this.getSchema(schemaCodes, schemaTypes, `IsMobilePhone`, 'string', schema)
          break;
        case 'email':
        ...
        case 'date':
          ...
          break;
        case 'boolean':
          this.getSchema(schemaCodes, schemaTypes, 'IsBoolean', 'boolean', schema)
          break;
        default:
          break;
      }
    ...
    const codes: string[] = []
    codes.push(`import { ApiProperty } from '@nestjs/swagger';`)
    codes.push(`import { ${uniqSchemaTypes.toString().replace(/,/g, `, `)} } from 'class-validator';`)
    ...
    codes.push('}')
    fs.writeFileSync(`${path}/${name}.dto.ts`, this.templateUtil.toString(codes, '\n'))
  }
}