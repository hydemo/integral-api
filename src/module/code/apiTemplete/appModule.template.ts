...
export class AppModuleTemplateService {
	...
	create(path: string, name: string) {
		...
		imports.push(
			`import { ${upCaseName}Module } from './module/${name}/${name}.module';`,
		);
		imports.push(
			`import { CMS${upCaseName}Controller } from './controller/cms/${name}.controller';`,
		);
		...
		appModule = appModule.replace(
			'// import锚点',
			this.templateUtil.toString(imports, '\n'),
		);
		appModule = appModule.replace(
			'// module锚点',
			this.templateUtil.toString(modules, '\n'),
		);
		appModule = appModule.replace(
			'// controller锚点',
			this.templateUtil.toString(controllers, '\n'),
		);
