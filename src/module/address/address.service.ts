...
export class AddressService {
  ...
  // 修改数据
  async findByIdAndUpdate(id: string, user: string, newAddress: CreateAddressDTO): Promise<Boolean> {
    const address = await this.addressModel.findById(id).lean().exec()
    if (!address) {
      return true
    }
  ...
  async findByUser(user: string): Promise<IAddress | null> {
    return await this.addressModel.findOne({ user, isDefault: true }).lean().exec()
  ...
  async setDefault(id: string, user: string): Promise<Boolean> {
    ...
    const defaultAddress = await this.addressModel.findOne({ user, isDefault: true })
    if (defaultAddress && String(defaultAddress._id) !== id) {
      await this.addressModel.findByIdAndUpdate(defaultAddress._id, { isDefault: false })
    }
