...
export class CollectService {
	...
	async create(createCollectDTO: CreateCollectDTO): Promise<ICollect> {
		const exist = await this.collectModel.findOne({
			user: createCollectDTO.user,
			bondToObjectId: createCollectDTO.bondToObjectId,
		});
		if (exist) {
			return exist;
		}
		...
		if (type === 1) {
			list = await Promise.all(
				collects.map(async collect => {
					collect.bondToObjectId.products = await this.productService.all({
						good: collect.bondToObjectId._id,
					});
					return collect;
				}),
			);
