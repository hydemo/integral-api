...
export class GoodService {
  constructor(
    ...
    @Inject(GoodSpecificationService) private readonly goodSpecificationService: GoodSpecificationService,
  ...
  async create(good: CreateGoodDTO, creator: IAdmin) {
    ...
    await this.goodModel.updateMany({ sort: { $gte: good.sort } }, { $inc: { sort: 1 } })
    const client = this.redis.getClient()
    await client.hincrby('dataRecord', 'goods', 1)
    const extendGood: any = { creator: creator._id, checkResult: 2, isMerchant: false }
    if (creator.role === 2) {
      extendGood.checkResult = 2
      extendGood.isMerchant = true
      extendGood.merchant = creator.merchant
    }
    const newGood = await this.goodModel.create({ ...good, ...extendGood })
    if (!good.goodSpecifications || !good.goodSpecifications.length) {
      return
    }
    const specifications: IGoodSpecification[] = await Promise.all(good.goodSpecifications.map(async spe => {
      return await this.goodSpecificationService.create({ ...spe, good: newGood._id })
    }))
    await Promise.all(good.products.map(async pro => {
      const productSpecifications: string[] = []
      pro.specifications.map((item, index) => {
        const select = specifications.find(o => o.indicator === item)
        if (!select) {
          throw new ApiException('Error Product', ApiErrorCode.INPUT_ERROR, 406)
        }
  ...
  async list(pagination: Pagination, admin: IAdmin): Promise<IList<IGood>> {
    ...
    if (admin.role === 2) {
      condition.merchant = admin.merchant
    } else {
      condition.checkResult = 2
    }
    const goods: IGood[] = await this.goodModel
      ...
      .lean()
      .exec();
    const list: IGood[] = await Promise.all(goods.map(async good => {
      good.products = await this.productService.all({ good: good._id })
      return good
    }))
    const total = await this.goodModel.countDocuments(condition);
    const max = await this.goodModel.countDocuments({ isDelete: condition.isDelete ? true : false })
    return { list, total, max };
  }

  // 商品列表
  async listByUser(pagination: Pagination, condition: any): Promise<IList<IGood>> {
    const newCondition = { ...condition, isDelete: false, onSale: true, checkResult: 2 }
    const goods: IGood[] = await this.goodModel
      .find(newCondition)
      .limit(pagination.pageSize)
      .skip((pagination.current - 1) * pagination.pageSize)
  ...
  async findByIdAndRemove(id: string): Promise<boolean> {
    const goodExist = await this.goodModel.findById(id).lean().exec()
    if (!goodExist) {
      throw new ApiException('分类有误', ApiErrorCode.INPUT_ERROR, 406)
    }
    await this.goodModel.findByIdAndUpdate(id, { isDelete: true, deleteTime: Date.now() })
    await this.goodModel.updateMany({ isDelete: false, sort: { $gte: goodExist.sort } }, { $inc: { sort: -1 } })
  ...
  async findByIdAndUpdate(id, good: CreateGoodDTO): Promise<boolean> {
    ...
    if (good.sort && goodExist.sort !== good.sort) {
      await this.goodModel.updateMany({ sort: { $gt: goodExist.sort } }, { $inc: { sort: -1 } })
      await this.goodModel.updateMany({ sort: { $gte: good.sort } }, { $inc: { sort: 1 } })
    }
  ...
  // 修改数据
  async updateProduct(good: string, product: UpdateProductDTO): Promise<boolean> {
    const { goodSpecifications, products } = product
    if (!products.length || !goodSpecifications.length) {
      throw new ApiException('规格不能为空', ApiErrorCode.INPUT_ERROR, 406)
    }
    await this.goodSpecificationService.softDelete({ good, _id: { $nin: goodSpecifications.map(v => v._id).filter(v => v) } })
    await this.productService.softDelete({ good, _id: { $nin: products.map(v => v._id).filter(v => v) } })

    const specifications: IGoodSpecification[] = await Promise.all(goodSpecifications.map(async spe => {
      ...
      if (spe._id) {
        const data = await this.goodSpecificationService.findByIdAndUpdate(spe._id, newSpe)
        if (!data) {
    ...
    await Promise.all(products.map(async pro => {
      ...
      if (pro._id) {
        return await this.productService.findByIdAndUpdate(pro._id, newProduct)
      }
  ...
  async recoverById(id: string) {
    ...
    await this.goodModel.updateMany({ isDelete: false, sort: { $gte: goodExist.sort } }, { $inc: { sort: 1 } })
    await this.goodModel.findByIdAndUpdate(id, { isDelete: false, $unset: { deleteTime: 1 } });
    return;
  }

  // 恢复
  async findById(id: string): Promise<IGood | null> {
    return await this.goodModel.findById(id).lean().exec();
  }

  // 增加销量
  async incSellVolumn(id: string, inc: number) {
    return await this.goodModel.findByIdAndUpdate(id, { $inc: { sellVolumn: inc } })
  }
  ...
  async detail(id: string): Promise<IGood | null> {
    ...
    const goodSpecifications = await this.goodSpecificationService.all({ good: good._id })
  ...
  async findByIdWithProduct(id: string) {
    ...
    const goodSpecifications: IGoodSpecification[] = await this.goodSpecificationService.all({ good: id })
    const specifications: any = []
    const specificationOptions = {}
    goodSpecifications.map((item) => {
      const index = specifications.findIndex(o => o._id === item.specification._id)
      if (index < 0) {
    ...
    return { ...good, specifications: specifications.sort((pre, next) => pre.sort - next.sort), specificationOptions, products }
  }
