...
export class UserRecordService {
  constructor(
    @Inject('UserRecordModelToken') private readonly userRecordModel: Model<IUserRecord>,
  ...
  async genRecord(user: string, order: IOrder) {
    ...
    if (recordExist) {
      await client.hset('userRecordBuy', String(user), (Number(recordExist) + price).toFixed(2))
    } else {
  ...
  async genLog() {
    ...
    await Promise.all(users.map(async user => {
      ...
      const preLog: IUserRecord | null = await this.userRecordModel.findOne({ newRecord: true, user })
      ...
      if (preLog) {
        await this.userRecordModel.findByIdAndUpdate(preLog._id, { newRecord: false })
      }
