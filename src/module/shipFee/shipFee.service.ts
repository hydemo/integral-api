...
export class ShipFeeService {
	...
	// 创建数据
	async create(
		createShipFeeDTO: CreateShipFeeDTO,
		user: string,
	): Promise<IShipFee> {
		return await this.shipFeeModel.create({
			...createShipFeeDTO,
			createBy: user,
		});
	}
	...
	async remove(id: string, user: string) {
		return await this.shipFeeModel.findByIdAndUpdate(id, {
			isDelete: true,
			deleteTime: Date.now(),
			deleteBy: user,
		});
	...
	async getShipFee(province: string, fee: number) {
		const shipFee: IShipFee | null = await this.shipFeeModel.findOne({
			province,
		});
		if (!shipFee) {
			return 0;
