...
export class UserCouponService {
	constructor(
		@Inject('UserCouponModelToken')
		private readonly userCouponModel: Model<IUserCoupon>,
	...
	async list(
		...
		switch (tab) {
			case 1:
				{
					condition.isUsed = false;
					condition.expire = { $gt: Date.now() };
				}
				break;
			case 2:
				{
	...
	async update(
		id: string,
		createUserCouponDTO: CreateUserCouponDTO,
	): Promise<IUserCoupon | null> {
		return await this.userCouponModel.findByIdAndUpdate(
			id,
			createUserCouponDTO,
		);
	}
	...
	async detail(id: string): Promise<IUserCoupon> {
		const userCoupon = await this.userCouponModel
			.findById(id)
			.lean()
			.exec();
		if (!userCoupon) {
	...
	async findOne(condition: any): Promise<IUserCoupon> {
		return await this.userCouponModel
			.findOne(condition)
			.lean()
			.exec();
	}
	...
	async remove(id: string, user: string) {
		return await this.userCouponModel.findByIdAndUpdate(id, {
			isDelete: true,
			deleteTime: Date.now(),
			deleteBy: user,
		});
	...
	async count(user: string) {
		const coupon = await this.userCouponModel.countDocuments({
			type: 1,
			isUsed: false,
			user,
			expire: { $gte: Date.now() },
		});
		const redbage = await this.userCouponModel.countDocuments({
			type: 2,
			isUsed: false,
			user,
			expire: { $gte: Date.now() },
	...
	async updateById(id: string, condition: any) {
		return await this.userCouponModel
			.findByIdAndUpdate(id, condition)
			.lean()
			.exec();
	}
	...
	async used(ids: string[]) {
		return await this.userCouponModel
			.updateMany({ _id: { $in: ids } }, { isUsed: true, useTime: Date.now() })
			.lean()
			.exec();
	}
}
