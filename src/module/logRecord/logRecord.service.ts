...
export class LogService {
  ...
  // 创建数据
  async genLog(url: string, method: string, user: string, ip: string, id: string): Promise<ILog | null> {
    ...
    // 评论特殊处理
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && urls[4] === 'feedback' && object.module === 'comment') {
    ...
    // 用户接口处理
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && urls[2] === 'users') {
      let type: string = ''
      switch (urls[4]) {
        case 'block':
          type = '禁用前台用户'
    ...
    // 退货退款特殊处理
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && object.module === 'refund') {
      let type: string = ''
      switch (urls[4]) {
        case 'confirm':
          type = '通过退货/退款申请'
    ...
    // 订单特殊处理
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && object.module === 'order') {
      let type: string = ''
      switch (urls[4]) {
        case 'send':
          type = '订单发货'
    ...
    // 商品特殊处理
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && urls[4] === 'product' && object.module === 'good') {
      const newLog: CreateLogDTO = {
        user,
        ip,
        module: object.module,
    ...
    if (method === 'PUT' && urls.length === 4 && urls[3] === 'bulk' && object.module === 'good') {
      const newLog: CreateLogDTO = {
        user,
        ip,
        module: object.module,
    ...
    if (method === 'PUT' && urls.length === 5 && validator.isMongoId(urls[3]) && urls[4] === 'recover') {
      const newLog: CreateLogDTO = {
        user,
        ip,
        module: object.module,
    ...
    if (method === 'DELETE' && urls.length === 4 && validator.isMongoId(urls[3])) {
      const newLog: CreateLogDTO = {
        user,
        ip,
        module: object.module,
  ...
  async list(pagination: Pagination): Promise<IList<ILog>> {
    const condition = this.paginationUtil.genCondition(pagination, ['interface'], 'createdAt');
