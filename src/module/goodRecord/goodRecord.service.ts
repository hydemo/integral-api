...
export class GoodRecordService {
	constructor(
		@Inject('GoodRecordModelToken')
		private readonly goodRecordModel: Model<IGoodRecord>,
	...
	async genRecord(good: string, product: IOrderProduct) {
		...
		const price = Number(
			(
				product.realPrice -
				product.vipMinus -
				product.promoteMinus -
				product.preSaleMinus -
				product.discountMinus
			).toFixed(2),
		);
		...
		if (goodDetail.category) {
			if (goodDetail.category[0]) {
				await this.categoryRecordService.genRecord(
					goodDetail.category[0],
					price,
					product.count,
					1,
				);
	...
	async genLog() {
		...
		await Promise.all(
			goods.map(async good => {
				...
				const preLog: IGoodRecord | null = await this.goodRecordModel.findOne({
					newRecord: true,
					good,
				});
	...
	// 获取排行
	async getRank() {
		const list = await this.goodRecordModel
			.find({ newRecord: true })
			.sort({ count: -1 })
			.limit(7)
