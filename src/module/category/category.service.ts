...
export class CategoryService {
	...
	async create(
		...
		if (category.layer === 1) {
			await this.categoryModel.updateMany(
				{ sort: { $gte: category.sort }, layer: 1 },
				{ $inc: { sort: 1 } },
			);
		} else {
			await this.categoryModel.updateMany(
				{ sort: { $gte: category.sort }, layer: 2, parent: category.parent },
				{ $inc: { sort: 1 } },
			);
		}
	...
	async findByIdAndRemove(id: string): Promise<boolean> {
		...
		await this.categoryModel.updateMany(
			{ isDelete: false, sort: { $gte: category.sort }, ...extendCondition },
			{ $inc: { sort: -1 } },
		);
	...
	async findByIdAndUpdate(id, category: CreateCategoryDTO): Promise<boolean> {
		const categoryExist = await this.categoryModel
			.findById(id)
			.lean()
			.exec();
		if (!categoryExist) {
		...
		if (category.sort && categoryExist.sort !== category.sort) {
			...
			await this.categoryModel.updateMany(
				{ sort: { $gt: categoryExist.sort }, ...extendCondition },
				{ $inc: { sort: -1 } },
			);
			await this.categoryModel.updateMany(
				{ sort: { $gte: category.sort }, ...extendCondition },
				{ $inc: { sort: 1 } },
			);
		}
	...
	async all() {
		...
		const categories = await this.categoryModel
			.find({ isDelete: false, layer: 1 })
			.sort({ sort: 1 });
		const data = await Promise.all(
			categories.map(async categoty => {
				const children = await this.categoryModel
					.find({ isDelete: false, parent: categoty._id, layer: 2 })
					.sort({ sort: 1 });
				if (!children.length) {
					return null;
				}
	...
	async secondLayer(parent: string) {
		return await this.categoryModel
			.find({ isDelete: false, layer: 2, parent })
			.sort({ sort: 1 });
	}
	...
	async top() {
		return await this.categoryModel
			.find({ showOnTop: true, isDelete: false, layer: 1 })
			.sort({ sort: 1 });
	}
}
