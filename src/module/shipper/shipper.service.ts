...
export class ShipperService {
  ...
  async traces(order: string) {
    const shipper: IShipper | null = await this.shipperModel.findOne({ order }).lean().exec()
    if (!shipper) {
    ...
    const expressInfo = await this.kdBirdUtil.queryExpress(shipper.shipperCode, shipper.logisticCode)
    if (expressInfo.success) {
      await this.shipperModel.findByIdAndUpdate(shipper._id, { isFinish: expressInfo.isFinish, traces: expressInfo.traces })
    }
