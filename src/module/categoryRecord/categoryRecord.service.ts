...
export class CategoryRecordService {
	constructor(
		@Inject('CategoryRecordModelToken')
		private readonly categoryRecordModel: Model<ICategoryRecord>,
	...
	async genLog() {
		...
		await Promise.all(
			categories.map(async category => {
				const sale = Number(
					await client.hget('categoryRecordSale_1', category),
				);
				const count = Number(
					await client.hget('categoryRecordCount_1', category),
				);
				const preLog: ICategoryRecord | null = await this.categoryRecordModel.findOne(
					{ newRecord: true, category },
				);
				...
				if (preLog) {
					await this.categoryRecordModel.findByIdAndUpdate(preLog._id, {
						newRecord: false,
					});
				}
	...
	async getRadar() {
		let other = 0;
		const datas: any[] = [];
		const categoryRecords = await this.categoryRecordModel
			.find({ newRecord: true })
			.sort({ sale: -1 })
		...
		categoryRecords.map((categoryRecord, index) => {
			if (index < 6 && categoryRecord.category) {
				return datas.push({
					...categoryRecord,
					category: categoryRecord.category.name,
				});
			}
