...
export class ProductService {
  ...
  async incSellVolumn(id: string, inc: number) {
    return await this.productModel.findByIdAndUpdate(id, { $inc: { sellVolumn: inc } })
  }
  ...
  async all(condition: any): Promise<IProduct[]> {
    ...
    return await this.productModel
      .find(condition)
      .populate({ path: 'specifications', model: 'goodSpecification', populate: { path: 'specification', model: 'specification' } })
      .lean()
      .exec()
  }
  ...
  async softDelete(condition: any): Promise<IProduct | null> {
    return await this.productModel.updateMany(condition, { isDelete: true, deleteTime: Date.now() })
  }

  // 修改数据
  async findByIdAndDelete(id: string): Promise<IProduct | null> {
    return await this.productModel.findByIdAndUpdate(id, { isDelete: true, deleteTime: Date.now() })
  }
