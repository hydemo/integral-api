...
export class GoodSpecificationService {
	constructor(
		@Inject('GoodSpecificationModelToken')
		private readonly goodSpecificationModel: Model<IGoodSpecification>,
	) {}

	// 创建数据
	async create(
		goodSpecification: CreateGoodSpecificationDTO,
	): Promise<IGoodSpecification> {
	...
	async findByIdAndUpdate(
		id: string,
		goodSpecification: CreateGoodSpecificationDTO,
	): Promise<IGoodSpecification | null> {
		return await this.goodSpecificationModel.findByIdAndUpdate(
			id,
			goodSpecification,
			{ new: true },
		);
	...
	async softDelete(condition: any): Promise<IGoodSpecification | null> {
		return await this.goodSpecificationModel.updateMany(condition, {
			isDelete: true,
			deleteTime: Date.now(),
		});
	}
