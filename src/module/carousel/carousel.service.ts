...
export class CarouselService {
	constructor(
		@Inject('CarouselModelToken')
		private readonly carouselModel: Model<ICarousel>,
	...
	// 全部数据
	async all() {
		const top = await this.carouselModel
			.find({ type: 1, isDelete: false })
			.lean()
			.exec();
		const middle = await this.carouselModel
			.find({ type: 2, isDelete: false })
			.lean()
			.exec();
		return { top, middle };
	}
