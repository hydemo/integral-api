...
export class CartService {
  ...
  async create(cart: CreateCartDTO, user: string): Promise<ICart | null> {
    const product: IProduct | null = await this.productServicee.findById(cart.product)
    if (!product) {
    ...
    if (exist) {
      return await this.cartModel.findByIdAndUpdate(exist._id, { $inc: { count: cart.count } })
    }
    const newCart = {
      // 用户
      user,
  ...
  // 增加购物车数量
  async incCount(id: string, count: CountDTO, user: string): Promise<ICart | null> {
    const cart = await this.cartModel.findById(id)
    if (cart && String(cart.user) !== String(user)) {
      throw new ApiException('无权限', ApiErrorCode.NO_PERMISSION, 403)
    }
