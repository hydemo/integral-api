...
import { IGoodRecord } from '../goodRecord/goodRecord.interfaces';
import { ICategoryRecord } from '../categoryRecord/categoryRecord.interfaces';

@Injectable()
export class DataRecordService {
  constructor(
    @Inject('DataRecordModelToken') private readonly dataRecordModel: Model<any>,
    ...
    @Inject(CategoryRecordService) private readonly categoryRecordService: CategoryRecordService,
    private readonly redis: RedisService,
  ) { }
  // 生成当天数据
  async genLog() {
    const preDate = moment().add(-2, 'd').format('YYYY-MM-DD')
    const preLog: IDataRecord | null = await this.dataRecordModel.findOne({ day: preDate })
  ...
  async getSummary(): Promise<{ record: IDataRecord, list: IDataRecord[] }> {
    ...
    const goodsTotal = await this.goodService.countByCondition({ isDelete: false })
    const categoriesTotal = await this.categoryService.countByCondition({ isDelete: false })
    const ordersTotal = await this.orderService.countByCondition({ isDelete: false, checkResult: { $gt: 1 } })
    const orderToSend = await this.orderService.countByCondition({ isDelete: false, checkResult: 2 })
    const orderSended = await this.orderService.countByCondition({ isDelete: false, checkResult: { $in: [3, 4] } })
    const record: IDataRecord = {
      ...data,
      usersTotal,
      viewCountTotal: preLog ? data.viewCount + preLog.viewCountTotal : data.viewCount,
      // 总用户数
      buyUsersTotal: preLog ? data.buyUsers + preLog.buyUsersTotal : data.buyUsers,
      categoriesTotal,
      goodsTotal,
      // 总销量
    ...
    const start = moment().add(-30, 'd').format('YYYY-MM-DD')
    const end = moment().add(-1, 'd').format('YYYY-MM-DD')

    const condition = { $and: [{ day: { $lte: end } }, { day: { $gte: start } }] }
  ...
  // 根据日期区间获取数据
  async getRecordBetween(startTime: string, endTime: string): Promise<IDataRecord[]> {
    const now = moment().format('YYYY-MM-DD')
    const start = moment(startTime).format('YYYY-MM-DD')
    const end = moment(endTime).format('YYYY-MM-DD')
    let condition: any;
    let data = await this.getToday();
    if (end === now) {
      const preDate = moment().add(-1, 'd').format('YYYY-MM-DD')
      condition = { $and: [{ day: { $lte: preDate } }, { day: { $gte: start } }] }
      if (start === end) {
        return [data]
      }
    } else {
