import { Body, Controller, Get, Param, Query, UseGuards, Inject, Request, Put, Response } from '@nestjs/common';
import {
  ApiUseTags,
  ApiOkResponse,
...
export class CMSOrderController {
  ...
  async download(
    ...
    const filename = await this.orderService.download('temp', Number(type), ids);
