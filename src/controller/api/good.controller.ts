...
export class ApiGoodController {
  constructor(
    ...
    @Inject(VipService) private vipService: VipService,
  ...
  async list(
    ...
    if (isLimit) {
      return await this.goodService.listByUser(pagination, { ...condition, isLimit: true, limitEndTime: { $gte: now } })
    }
    if (preSale) {
      return await this.goodService.listByUser(pagination, { ...condition, preSale: true, preSaleEndTime: { $gte: now } })
    }
    if (isHot) {
      return await this.goodService.listByUser(pagination, { ...condition, isHot: true })
    }
    if (isNewUser) {
      return await this.goodService.listByUser(pagination, { ...condition, isNewUser: true })
    }
  ...
  async vip(
    ...
    return await this.vipService.findMaxPromote()
  }
  ...
  async findById(
    ...
    const comments = await this.commentService.findByBoundToObjectId({ current: 1, pageSize: 10 }, id)
    let isCollect = false
    ...
    if (headers.authorization) {
      ...
      if (token) {
        ...
        if (payload.type === 'user') {
          const collect = await this.collectService.findOne({ user: payload.id, bondToObjectId: id, bondType: 1 })
