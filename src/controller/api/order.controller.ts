...
import { CreateOrderByProductDTO, CreateOrderByCartDTO, ConfirmOrderDTO } from 'src/module/order/order.dto';
...
export class ApiOrderController {
  ...
  @ApiOperation({ title: '订单列表', description: '订单列表 状态 0:待确认 1:待付款 2:待发货 3：待收货 4:待评价 5:完成 6:退款' })
  async list(
    @Query() pagination: Pagination,
    @Request() req: any,
    @Query('checkResult') checkResult?: number,
  ): Promise<any> {
    return await this.orderService.listByUser(pagination, req.user._id, Number(checkResult))
  }
  ...
  async pay(
    ...
    return await this.orderService.payOrder(id, req.user)
  }
  ...
  @ApiOperation({ title: '余额支付订单', description: '余额支付订单' })
  async payByBalance(
    @Param('id', new MongodIdPipe()) id: string,
    @Request() req: any,
  ): Promise<any> {
    return await this.orderService.payOrderByBalance(id, req.user)
  }
  ...
  @ApiOperation({ title: '获取可用红包', description: '获取可用红包' })
  async coupons(
    @Param('id', new MongodIdPipe()) id: string,
    @Query('type') type: number,
    @Request() req: any,
  ): Promise<any> {
    return await this.orderService.orderCoupon(id, type, req.user._id)
  }
