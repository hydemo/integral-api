...
export class ApiRefundController {
  ...
  @ApiOperation({ title: '退货退款列表', description: '状态 1:待审核 2:待发货 3:待退款 4:完成 5:已拒绝' })
  async list(
    @Query() pagination: Pagination,
    @Request() req: any,
    @Query('checkResult') checkResult?: number,
  ): Promise<any> {
    return await this.refundService.listByUser(pagination, req.user._id, Number(checkResult))
  }
