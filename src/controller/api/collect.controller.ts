import { Body, Controller, Get, Param, Query, UseGuards, Inject, Request, Delete } from '@nestjs/common';
import {
  ApiUseTags,
  ApiOkResponse,
...
export class ApiCollectController {
  ...
  async list(
    ...
    return await this.collectService.list(pagination, req.user._id, Number(type))
  }
  ...
  async deleteMany(
    ...
    return await this.collectService.deleteMany({ _id: { $in: collects.collects }, user: req.user._id })
  }
