...
export class WeixinUtil {
  ...
  async login(code: string, login: LoginDTO): Promise<any> {
    try {
      ...
      // 验证用户信息完整性
      const sha1 = crypto.createHash('sha1').update(login.rawData.toString() + sessionData.session_key).digest('hex');
      if (login.signature !== sha1) {
        return null;
      }

      // 解析用户数据
      const wechatUserInfo = await this.decryptUserInfoData(sessionData.session_key, login.encryptedData, login.iv);
      if (!wechatUserInfo) {
        return null;
      }
   ...
   * @returns {Promise.<any>}
   */
  async decryptUserInfoData(sessionKey: string, encryptedData: string, iv: string): Promise<any> {
    let decoded = '';
    try {
      ...
      weixinpay.createUnifiedOrder({
        ...
        if (res.return_code === 'SUCCESS' && res.result_code === 'SUCCESS') {
          ...
          const paramStr = `appId=${returnParams.appid}&nonceStr=${
            returnParams.nonceStr}&package=${
            returnParams.package}&signType=${
            returnParams.signType}&timeStamp=${
            returnParams.timeStamp}&key=${
   ...
   * @returns {Promise}
   */
  async refund(fee: number, totalFee: number, openId: string, order: string, refund: string) {
    const pfx = fs.readFileSync('apiclient_cert.p12')
    const config = {
      appid: this.config.appid,
      mchid: this.config.mchId,
